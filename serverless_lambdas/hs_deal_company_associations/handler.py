...
NETSUITE_DEAL_PROP = "bill_to_customer_id"
NETSUITE_COMPANY_PROP = "bc_unique_id_2"



# === FETCH CONTACTS ======================================================

def get_deals(weeks_back=24000) -> List[Dict]:
    ...
    while True:
        ...
        params = {
            ...
            "properties": [NETSUITE_DEAL_PROP,"lastmodifieddate", "sales_order_id"],
            ...
            "filterGroups": [
                {
                    "filters": [
                        ...
                        {
                            "propertyName": "bill_to_customer_id",
                            "operator": "HAS_PROPERTY",
                        }
                    ]
                }
        ...
        for c in data.get("results", []):
            lastmod = c["properties"].get("hs_lastmodifieddate")
            if not lastmod:
                continue
...
def build_company_map(unassoc_contacts: List[Dict]) -> Dict[str, str]:
    netsuite_ids = {
        c["properties"].get(NETSUITE_DEAL_PROP)
        for c in unassoc_contacts
        if c["properties"].get(NETSUITE_DEAL_PROP)
    }
    ...
    for i in range(0, len(ids), 100):
        ...
        payload = {
            ...
            "limit": 100,
            "properties": [NETSUITE_COMPANY_PROP],
        }
...
def link_deals_to_companies(unassoc_contacts: List[Dict]) -> int:
    company_map = build_company_map(unassoc_contacts)

    associations = []
    for c in unassoc_contacts:
        ns_id = c["properties"].get(NETSUITE_DEAL_PROP).strip()
    ...
    for i in range(0, len(associations), 100):
        ...
        r = requests.post(
            f"{HS_BASE}/crm/v4/associations/company/deal/batch/create",
            headers=HEADERS,
            json={"inputs": batch},
        )
