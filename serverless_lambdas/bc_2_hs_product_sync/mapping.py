...
def map_products(person: Dict[str, Any]) -> Dict[str, Any]:
    mapped = generic_map(person)
    item_number = mapped.get("item_number")
    mapped["hs_product_type"] = mapped.get("hs_product_type", "").lower() if mapped.get("hs_product_type") else Noneç

    try:

        if item_number in mapped.get("description", ""):
            item_details = mapped.get("description").split(item_number)[1]
            item_details = item_details.strip()

            if " " in item_details:
                color = item_details.split(" ")[0]
                size = item_details.split(" ")[1]
                ...
                mapped["color"] = color
                mapped["size"] = size
        elif "size" in mapped["description"].lower():
            size = mapped["description"].split("Size")[1].strip()
            mapped["size"] = size
        ...
        print(f"Error parsing item details for item_number '{item_number}': {e}")
        mapped["color"] = None
        mapped["size"] = None
    return mapped
