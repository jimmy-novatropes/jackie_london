...
def prepare_products_batch_payload(
    product_list: List[Dict[str, str]], unique_prop: str = "sku"
) -> Tuple[str, Dict]:
    """
    Prepares the payload and URL for upserting multiple products in HubSpot.

    Args:
        product_list (list[dict]): List of product records, each with properties.
                                    Each record must include the unique_prop (default: "sku").
        unique_prop (str): The unique property to identify existing records (e.g., "sku").

    Returns:
        tuple: (url, payload)
            - url (str): HubSpot batch upsert endpoint for products
            - payload (dict): JSON body formatted for HubSpot API
    Example:
        products = [
        ...
        url, payload = prepare_products_batch_payload(products)
    """
    url = f"{HUBSPOT_API}/crm/v3/objects/products/batch/upsert"
    inputs = [
        {"idProperty": unique_prop, "id": p[unique_prop], "properties": p}
